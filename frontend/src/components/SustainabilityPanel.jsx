...
export default function SustainabilityPanel() {
  ...
  return (
    <div className="space-y-5">
      {/* ═══ HERO HEADER ═══ */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-[#0c1a2e] to-slate-900 border border-emerald-500/10">
        {/* Background ambient */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute -right-20 -top-20 w-80 h-80 rounded-full bg-emerald-500/[0.07] blur-3xl" />
        ...
        <div className="relative z-10 px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-white tracking-tight">Sürdürülebilirlik & Emisyon</h1>
        ...
        <motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-5">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-base font-bold text-white">Model Bazlı Karbon Emisyon</h2>
                ...
                return (
                  <motion.div key={m.model} layout
                    className={`rounded-xl border transition-all duration-300 overflow-hidden cursor-pointer ${
                      isExpanded ? 'col-span-3 border-emerald-500/30 bg-slate-800/60' : 'border-white/10 bg-slate-900/60 hover:border-emerald-500/20'
                    }`}
                    onClick={() => setExpandedModel(isExpanded ? null : m.model)}>
                    <div className="p-4">
                      <div className="flex items-center gap-4">
                        {/* Bus image */}
                        <div className="w-24 h-16 rounded-lg bg-slate-800/60 overflow-hidden flex items-center justify-center flex-shrink-0">
                          {img ? (
                          ...
                          <div className={`${img ? 'hidden' : 'flex'} items-center justify-center w-full h-full`}>
                            <svg viewBox="0 0 24 24" className="w-10 h-10 text-slate-600" fill="none" stroke="currentColor" strokeWidth="1.2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h3 className="text-sm font-bold text-white">{m.model}</h3>
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          <div className="text-[9px] text-slate-500 uppercase">g/km ort.</div>
                        </div>
                        {/* Min-Max */}
                        <div className="text-right flex-shrink-0 pl-3 border-l border-white/5">
                          ...
                          <div className="text-[8px] text-slate-600">min / max</div>
                        </div>
                        {/* Expand icon */}
                        <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} className="text-slate-500 ml-2">
                    ...
                    <AnimatePresence>
                      {isExpanded && modelVehicles.length > 0 && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                          className="overflow-hidden border-t border-white/5" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-white">Model CO₂ Ortalaması</h3>
                </div>
              ...
              <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-white">Araç Dağılımı</h3>
                </div>
                  ...
                  <div className="flex-1 space-y-2">
                    {ms.map((m, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-slate-300 flex-1 truncate">{m.model}</span>
                        <span className="text-[10px] font-bold text-slate-400">{m.vehicle_count}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {section === 'import' && (
        <motion.div key="import" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
          ...
          <div className="rounded-xl border border-white/10 bg-slate-900/60 p-6">
            ...
            <label className="block">
              <input type="file" accept=".xml" multiple onChange={handleFileUpload} disabled={importing}
                className="block w-full text-xs text-slate-400 file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-gradient-to-r file:from-cyan-500 file:to-blue-500 file:text-white file:cursor-pointer file:shadow-lg file:shadow-cyan-500/25 hover:file:shadow-cyan-500/40 file:transition-all" />
            </label>
          </div>
          ...
          {importResult && (
            <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5">
              <h3 className="text-sm font-bold text-white mb-3">Aktarım Sonucu</h3>
...
function KpiCard({ label, value, unit, color }) {
  return (
    <div className="rounded-xl border border-white/10 bg-slate-900/60 p-3.5 text-center">
      ...
      {unit && <div className="text-[9px] text-slate-600">{unit}</div>}
    </div>
  );
}
...
function HeroVehicleCard({ vehicle, type }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: isBest ? -20 : 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: isBest ? 0.1 : 0.2 }}
      className={`relative overflow-hidden rounded-xl border ${borderColor} bg-gradient-to-br ${gradFrom} via-transparent ${gradTo} p-5`}>
      <div className="flex items-center gap-5">
        {/* Bus image */}
        <div className="w-28 h-20 rounded-lg bg-slate-800/40 overflow-hidden flex items-center justify-center flex-shrink-0">
          {img ? (
            <img src={img} alt={vehicle.model} className="w-full h-full object-contain opacity-90" />
          ) : (
            <svg viewBox="0 0 24 24" className="w-12 h-12 text-slate-600" fill="none" stroke="currentColor" strokeWidth="1">
              <rect x="2" y="6" width="20" height="12" rx="3" /><circle cx="7" cy="18" r="1.5" /><circle cx="17" cy="18" r="1.5" />
            </svg>
          )}
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="text-lg font-bold text-white">{vehicle.model}</div>
          <div className="text-[10px] text-slate-400 font-mono mt-0.5">{vehicle.vin}</div>
  ...
  return (
    <div className="rounded-lg border border-white/5 bg-slate-800/30 p-3">
      <div className="flex items-center gap-3">
        <div className="w-14 h-9 rounded bg-slate-800/60 overflow-hidden flex items-center justify-center flex-shrink-0">
          {img ? <img src={img} alt={v.model} className="w-full h-full object-contain opacity-70" /> : null}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[10px] font-mono text-slate-300 truncate">{v.vin}</div>
...
function Co2Tooltip({ active, payload }) {
  ...
  return (
    <div className="bg-slate-900/95 backdrop-blur-md border border-white/10 rounded-xl p-3 shadow-2xl">
      ...
      <div className="text-[11px] text-emerald-400 font-semibold">{d.co2_avg} g/km</div>
...
function EmptyBox({ msg, sub }) {
  return (
    <div className="rounded-xl border border-white/5 bg-slate-900/40 p-10 text-center">
      <svg viewBox="0 0 24 24" className="w-10 h-10 mx-auto text-slate-700 mb-3" fill="none" stroke="currentColor" strokeWidth="1.2">
        <rect x="2" y="6" width="20" height="12" rx="3" /><circle cx="7" cy="18" r="1.5" /><circle cx="17" cy="18" r="1.5" />
        <line x1="6" y1="9" x2="6" y2="13" /><line x1="10" y1="9" x2="10" y2="13" /><line x1="14" y1="9" x2="14" y2="13" /><line x1="18" y1="9" x2="18" y2="13" />
      </svg>
